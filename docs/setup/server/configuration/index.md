...
{{ project.name }} is configured via a JSON file. Use the `CONFIG_PATH` [environment variable](env.md) to set the path
to the JSON file.
...
See [GIF integration documentation](./integrations/gifs.md) for more up to date configuration guidance for GIF
providers.

| key                               | default   | type    | description                               |
|-----------------------------------|-----------|---------|-------------------------------------------|
| `integrations.gifs.klipy.enabled` | false     | boolean | Whether the Klipy GIF provider is enabled |
| `integrations.gifs.klipy.apiKey`  | undefined | string? | Klipy API key                             |
| `integrations.gifs.klipy.enabled` | undefined | string? | Path to file containing Klipy API key     |
